...
class TestPerformanceBenchmarks:
    ...
    def test_sidebar_loading_performance(self) -> None:
        ...
        assert metrics.memory_usage < 50 * 1024 * 1024, (  # 50MB
            f"Sidebar loading used {metrics.memory_usage} bytes, "
            "exceeding reasonable memory limit"
        )

        print(f"Sidebar loading: {metrics.execution_time:.4f}s, "
              f"{metrics.memory_usage} bytes")
    ...
    def test_theme_state_operations_performance(self) -> None:
        ...
        print(f"State operations - Load: {load_metrics.execution_time".4f"}s, "
    ...
    def test_menuitem_creation_performance(self) -> None:
        ...
        for label, target in test_cases:
            ...
            assert metrics.execution_time < 0.1, (
                f"MenuItem creation took {metrics.execution_time".4f"}s, "
                "exceeding reasonable time"
            )
        ...
        for menu in sidebar.navbar:
            for item in menu.children:
                ...
                assert metrics.execution_time < 0.01, (
                    f"Progress calculation took {metrics.execution_time".4f"}s, "
                    "exceeding reasonable time"
                )
        ...
        for file_path in test_files:
            if os.path.exists(file_path):
                ...
                assert metrics.execution_time < 0.5, (  # 500ms should be plenty
                    f"Localization loading took {metrics.execution_time".4f"}s, "
                    "exceeding reasonable time"
                )
    ...
    def test_complete_workflow_performance(self) -> None:
        ...
        print(f"Complete workflow: {metrics.execution_time".4f"}s, "
              f"{metrics.memory_usage} bytes")
    ...
    def test_memory_efficiency(self) -> None:
        ...
        assert memory_increase < 100 * 1024 * 1024, (  # 100MB
            f"Memory increase of {memory_increase} bytes is too high"
        )
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            ...
            for future in as_completed(sidebar_futures):
                ...
                progress_futures.append(
                    executor.submit(calculate_progress, sidebar)
                )
    ...
    def test_scalability_performance(self) -> None:
        ...
        assert scalability_time < 5.0, (
            f"Large menu operations took {scalability_time".4f"}s, "
    ...
    def test_cold_start_performance(self) -> None:
        ...
        assert cold_start_metrics.execution_time < TestConfig.MAX_EXECUTION_TIME * 2, (
            f"Cold start took {cold_start_metrics.execution_time".4f"}s, "
            "too slow for initial load"
        )
