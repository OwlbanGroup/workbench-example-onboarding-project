...
def benchmark_performance(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs) -> Any:
        ...
        metrics = PerformanceMetrics(
            execution_time=execution_time, memory_usage=memory_usage, function_name=func.__name__
        )

        # Log performance metrics
        print(f"Performance: {func.__name__} took {execution_time:.4f}s, " f"memory delta: {memory_usage} bytes")
